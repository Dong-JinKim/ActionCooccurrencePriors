...
def select_best_boxes_across_dataset(
    ...
    for global_id in tqdm(pred_hois.keys()):
        ...
        for i in range(600):
            ...
            #pdb.set_trace()
            if boxes is not None and len(global_id_det_id_score[hoi_id])<exp_const.num_to_vis:#---if ther is more space, add.
                max_ind = np.argmax(human_obj_boxes_scores[start_id:end_id ,8])
                j = range(start_id,end_id)[max_ind]
                ...
                global_id_det_id_score[hoi_id].append(
                    (
                        ...
                        boxes,
                        indexes,
                        human_obj_boxes_scores[start_id:end_id ,8],
                    )
                )
    ...
    for hoi_id in tqdm(global_id_det_id_score.keys()):
        ...
        for i in range(exp_const.num_to_vis):
            if i>=len(global_id_det_id_score[hoi_id]):
                break
            global_id,det_id,score,boxes,indexes,other_predictions = global_id_det_id_score[hoi_id][i]
            ...
            top_predictions.append(sorted(other_predictions,reverse=True)[:10])
...
def get_pred_hois(pred,hoi_dict):#----!!!!
    gt_hoi_names = ''
    #pdb.set_trace()
    for iid in range(min(10,len(pred))):
        #hoi_id = pred[iid][0]
        score = pred[iid]
        if score>1e-8:
            #obj_name = hoi_dict[hoi_id]['object']
...
def create_html(top_boxes,anno_dict,hoi_dict,img_dir,vis_dir):
    for hoi_id in tqdm(top_boxes.keys()):
        ...
        col_dict = {
            0: 'Global ID',
            1: 'Predicted Score',
            2: 'All GT HOI categories annotated in the image <br /> (for any human-object pair)',
            3: f'GT Detections for {hoi_name}',
            4: f'Predictions for {hoi_name}',
            5: f'Detected Boxes and Pose for human and {object_name} categories',
        ...
        for i, global_id in enumerate(top_boxes[hoi_id]['global_ids']):
            ...
            col_dict = {
                ...
                4: html_writer.image_tag(str(i).zfill(3)+'.png'),
                5: html_writer.image_tag(str(i).zfill(3)+'_on_white.png'),
                6:pred_hoi_names,#----!!!!
            }
            html_writer.add_element(col_dict)
...
def main(exp_const,data_const,model_const):
    ...
    vis_dir = os.path.join(exp_const.exp_dir,'vis/recall_boxes_per_hoi')
    create_html(
        top_boxes,
        anno_dict,
        hoi_dict,
