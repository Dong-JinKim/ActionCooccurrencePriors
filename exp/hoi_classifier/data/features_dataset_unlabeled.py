...
class FeatureConstants(HicoConstants,io.JsonSerializableClass):
    def __init__(
            ...
            proc_dir=os.path.join(os.getcwd(),'data_symlinks/hico_processed_finetune101_unlabeled'),
            hoi_cand_dir=os.path.join(
                os.getcwd(),
                'data_symlinks/hico_exp_finetune101_unlabeled/hoi_candidates')):
...
class Features_unlabeled(Dataset):
    def __init__(self,const):
        self.const = copy.deepcopy(const)
        self.hoi_cands = self.load_hdf5_file('/home/dongjin/Documents/no_frills_hoi_det/data_symlinks/hico_exp_finetune101_unlabeled/hoi_candidates/hoi_candidates_unlabeled.hdf5')

        #self.hoi_cand_labels = self.load_hdf5_file(
        #    self.const.hoi_cand_labels_hdf5)
        self.faster_rcnn_feats = self.load_hdf5_file('/home/dongjin/Documents/no_frills_hoi_det/data_symlinks/hico_processed_finetune101_unlabeled/faster_rcnn_fc7.hdf5')
        #self.global_ids = self.load_subset_ids(self.const.subset)
        self.global_ids = io.load_json_object('unlabeled_ids.json')#--------!!!!!!
        ...
        if self.const.box_feats_hdf5:
            self.box_feats = self.load_hdf5_file('/home/dongjin/Documents/no_frills_hoi_det/data_symlinks/hico_exp_finetune101_unlabeled/hoi_candidates/hoi_candidates_box_feats_unlabeled.hdf5')
        if self.const.human_pose_feats_hdf5:
            self.human_pose_feat = self.load_hdf5_file('/home/dongjin/Documents/no_frills_hoi_det/data_symlinks/hico_exp_finetune101_unlabeled/hoi_candidates/human_pose_feats_unlabeled.hdf5')
    ...
    def __getitem__(self,i):
        ...
        hoi_ids_= self.get_labels(global_id)#------!!!!!
        if self.const.box_feats_hdf5:
